...
| --- | --- | --- | --- | --- |
| RTS missing `action_token` | `assistant.search.context` returns auth or token error | Required tech is fragile in demo | Use `app_mention` as the main trigger and log whether token exists | Load `mockContext.json`; show "RTS fallback" badge |
| RTS scopes incomplete | Search returns missing scope or empty results | Live context unavailable | Request `search:read.public`, `app_mentions:read`, `chat:write`, and verify RTS-specific setup early | Seed context JSON mirrors Slack messages |
