...
const normalizeRtsMessage = (item: any, index: number): Evidence => {
  ...
  return {
    ...
    text: String(item.content ?? item.text ?? item.message?.text ?? item.title ?? "Slack context result"),
...
const isRelevant = (text: string, zoneName: string): boolean => {
  ...
  return lower.includes(zoneName.toLowerCase()) || CRISIS_TERMS.some((term) => lower.includes(term));
};

const buildContextQuery = (zoneName: string): string => {
  return `"${zoneName}" (flood OR rain OR water OR route OR shelter OR volunteer OR supplies OR evacuation OR blocked)`;
};
...
const searchRealtimeContext = async (client: WebClient, actionToken: string, zoneName: string): Promise<Evidence[]> => {
  ...
  return Array.isArray(rawMessages) ? rawMessages.map(normalizeRtsMessage).slice(0, 4) : [];
};
